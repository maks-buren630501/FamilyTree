...
async def registration_user(user: UserSchemaCreate, crud: UserCrud = Depends(user_crud)) -> str:
    try:
        ...
        mail.send_message(user.email, f"Subject: Activate account FamilyTree\nGo to link '127.0.0.1/{new_user}'")
        return new_user
    except UniqueIndexException as e:
        raise NotUniqueIndex(e)


@app_authentication.put('/activate/{user_id}', **activate_user_url_config.dict())
async def activate_user(user_id: str, crud: UserCrud = Depends(user_crud)):
    update_count = await crud.update(user_id, {'active': True})
    if update_count:
        return update_count
    else:
        return Response(status_code=status.HTTP_204_NO_CONTENT)
